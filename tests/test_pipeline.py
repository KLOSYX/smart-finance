...
from src.llm_client import analyze_transactions, generate_financial_advice

class TestPipeline(unittest.TestCase):
    
    def setUp(self):
        self.dummy_pdf_path = "tests/dummy_statement.pdf"
    ...
    def test_llm_analysis(self, mock_openai):
        ...
        [
            {"Date": "2023-10-01", "Description": "UBER TRIP", "Amount": 25.50, "Category": "Transportation"},
            {"Date": "2023-10-02", "Description": "STARBUCKS", "Amount": 5.40, "Category": "Food & Dining"}
        ]
        """
        mock_client.chat.completions.create.return_value = mock_completion
        
        text = "Simulated extracted text"
        result = analyze_transactions(text, "fake-key", "fake-url", "gpt-3.5")
    ...
    @patch('src.llm_client.OpenAI')
    def test_financial_advice(self, mock_openai):
        mock_client = MagicMock()
        mock_openai.return_value = mock_client
        
        mock_completion = MagicMock()
        mock_completion.choices[0].message.content = "Stop spending so much on coffee."
        mock_client.chat.completions.create.return_value = mock_completion
        ...
        self.assertEqual(advice, "Stop spending so much on coffee.")
